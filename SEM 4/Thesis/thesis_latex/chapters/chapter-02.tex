...
\subsection{Recommender System Paradigms and the Classification-Based Approach}

Before reviewing the specific literature on machine learning in telecom bundle recommendation, it is necessary to situate this study within the broader taxonomy of recommender system approaches. Recommender systems are classified into three established paradigms: Collaborative Filtering (CF), Content-Based Filtering, and Hybrid Systems \cite{ref1, ref19}.

Collaborative Filtering generates recommendations by identifying patterns in user--item interaction histories---recommending items that users with similar profiles have engaged with \cite{ref26, ref40}. However, CF requires dense interaction matrices and suffers from the cold-start problem. In the prepaid telecom context, the subscriber-to-bundle interaction space is extremely sparse---each subscriber typically purchases only a small fraction of the 738 available bundle products---making CF structurally unsuitable for this setting.

Content-Based Filtering recommends items similar in attributes to those previously consumed. In a telecom context, this would recommend bundles similar in price or duration to prior purchases. While appropriate in some contexts, this approach risks over-specialisation and cannot detect behavioural signals indicating readiness for a higher-tier product.

Hybrid Recommender Systems combine collaborative and content-based signals \cite{ref19}, but require both rich interaction histories and item attribute data---conditions not fully met in the prepaid subscriber context.

Classification-Based Recommendation, the approach adopted in this study, represents a fourth paradigm that has gained traction where rich behavioural feature sets are available but interaction matrices are sparse \cite{ref53, ref44}. In this paradigm, a supervised classifier predicts the value segment that best characterises a subscriber's behavioural profile, and the segment prediction is used as the targeting rule for bundle recommendations: the classifier predicts that Subscriber A belongs to the Platinum segment, and the recommendation engine surfaces the Platinum tier's bundle portfolio. The justification for this approach is threefold:
\begin{enumerate}
    \item Rich behavioural OCS features are available but interaction matrices are too sparse for CF;
    \item The classification output is directly consumable by Airtel Uganda's existing USSD rules engine without new system integration;
    \item Classification-based recommendation is validated in the telecom literature \cite{ref8, ref44}.
\end{enumerate}
...
Table~\ref{tab:litreview} reveals several important patterns and gaps across the reviewed literature that directly motivate the present study. First, ensemble
methods --- particularly Random Forest and XGBoost --- are the most consistently high-performing models for telecom classification tasks, with reported F1-Scores
ranging from 0.78 to 0.92 across the reviewed studies  (Chang et al., 2024; Alotaibi \& Haq, 2024; Sikri et al., 2024) \cite{ref11, ref8, ref43}.
Logistic Regression and single Decision Trees consistently underperform ensemble approaches. Second, Collaborative Filtering approaches achieve strong recommendation
accuracy in data-rich global market contexts, but are predominantly applied where dense interaction histories exist --- a condition not met in the Ugandan prepaid
setting. Third, and most critically, none of the reviewed studies were conducted using real proprietary transaction data from a Ugandan telecommunications operator
--- all rely either on publicly available benchmark datasets (notably the IBM Telco Churn dataset) or on synthetically generated samples. This constitutes the primary
gap that the present study addresses. Furthermore, none of the reviewed studies simultaneously address voice bundle recommendation, value segment classification, and business impact projection within a single analytical framework --- a combination this study contributes to the literature.
